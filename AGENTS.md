...
- `composer.lock` is gitignored but checked in; `package-lock.json` is gitignored and not checked
  in.
- PHP config `platform.php: "8.3"` — won't install on older PHP, even for dev.

## Tooling

- **php-cs-fixer**: PSR-12 base, strict types, ordered class elements
  (traits→constants→properties→constructor→methods), no unused imports, import
  classes/constants/functions, short array syntax
- **phpstan**: level 8 (strictest), scans `src/` and `tests/`, excludes `var/` and `vendor/`
- **rector**: PHP 8.3, dead code + code quality + coding style + type declarations + early return +
  phpunit sets, Symfony composer-based rules, `DisallowedEmptyRuleFixerRector` skipped
- **markdownlint**: max line length 100, ATX headers, fenced code blocks
- **prettier**: 4-space indent, single quotes, trailing commas, LF endings; MD files overridden to
  2-space/100-char print width
